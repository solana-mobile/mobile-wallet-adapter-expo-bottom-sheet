...
export class ClientTrustUseCase {
    ...
    switch (this.associationType) {
      case AssociationType.LocalFromBrowser:
        if (clientIdentityUri != null) {
          ...
          console.debug(
            "Client did not provide an identity URI; not verifiable"
          );
  ...
  async verifyPrivaledgedMethodSource(
    authorizationScope: string,
    clientIdentityUri?: string
  ): Promise<boolean> {
    if (authorizationScope.startsWith(AssociationType.LocalFromBrowser)) {
      if (this.associationType != AssociationType.LocalFromBrowser) {
        ...
        ClientTrustUseCase.SCOPE_DELIMITER
      ) {
        console.warn(
          `Unexpected character '${authorizationScope[AssociationType.LocalFromBrowser.length]}' in scope; expected '${ClientTrustUseCase.SCOPE_DELIMITER}'`
        );
        return false;
      } else {
        console.debug(
      ...
      if (this.associationType != AssociationType.LocalFromApp) {
        ...
        console.warn(
          `Unexpected character '${authorizationScope[AssociationType.LocalFromApp.length]}' in scope; expected '${ClientTrustUseCase.SCOPE_DELIMITER}'`
        );
        return false;
      } else {
        var scopeUid: number;
